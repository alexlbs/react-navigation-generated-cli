...
var TYPE_IMPORT_MATCHER = /import.*{.*[A-Za-z] as (.*)Params.*}.*(?="|').*(?="|')/g;
...
var writeRouteParamTypes = function (appRouteFilePath, rootNavigators) {
    ...
    var fragmentParams = {};
    var routeToImport = {};
    var keyToImports = (0, match_all_1.default)(fileData, TYPE_IMPORT_MATCHER)
        .toArray()
        .map(function (s) {
        var sfrag = s.split('_');
    ...
    var noParamsString = __spreadArray([], __read(allRouteNames), false).filter(function (k) { return !routeNamesWithParams.has(k); })
        .reduce(function (str, routeName) { return str + ("  '" + routeName + "': {};\n"); }, '');
    var paramsString = __spreadArray([], __read(fragmentRouteNames), false).reduce(function (str, key, i) { return str + (i === 0 ? '\n' : '') + ("  " + key + " = '" + key + "', \n"); }, '');
    var fragString = __spreadArray([], __read(fragmentRouteNames), false).filter(function (k) { return !routeNamesWithParams.has(k); })
...
try {
    ...
    if (navigationroot_1 && outputpath_1) {
        ...
        var onExpoData_1 = function (data) {
            ...
            var outputHasIdentifier = output.includes(START_IDENTIFIER);
            var outputHasEndIdentifier = output.includes(END_IDENTIFIER);
            if (commander_1.default.showLogs &&
                !outputHasIdentifier &&
                output.length < 1000 &&
                output &&
                output.trim()) {
            ...
            if (outputHasIdentifier) {
                routeMapString_1 += output.match(/REACT_NAVIGATION_GENERATED_OUTPUT:(.*)/)[1];
            }
            if (outputHasEndIdentifier && (!finished_1 || commander_1.default.keepOpen)) {
                finished_1 = true;
                if (!commander_1.default.keepOpen) {
                    expoProcess_1.kill();
                }
                ...
                try {
                    var parsedMap = JSON.parse(routeMapString_1);
                    if (routeMapString_1 === prevRouteMap_1)
                        return;
                    prevRouteMap_1 = routeMapString_1;
                    var outputPath = process.cwd() + outputpath_1;
                    var tsString = "const routeMap = " + routeMapString_1 + " as const;export default routeMap;";
        ...
        var expoProcess_1 = (0, child_process_1.exec)('expo start -i');
        var initExpoProcess_1 = function () {
            ...
            expoProcess_1.on('exit', function () {
                ...
                expoProcess_1 = (0, child_process_1.exec)('expo start -i');
                initExpoProcess_1();
            });
        };
        if (expoProcess_1) {

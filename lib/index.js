...
var writeRouteParamTypes = function (appRouteFilePath, rootNavigators) {
    ...
    var typesObjString = keyToImports.map(function (_a) {
    ...
    var noParamsString = __spread(allRouteNames).filter(function (k) { return !routeNamesWithParams.has(k); })
        .map(function (routeName) { return "  '" + routeName + "': {};"; }).join('\n');
    //.reduce((str, routeName) => str + `  '${routeName}': {};\n`, '');
    //  const paramsString = [...fragmentRouteNames].reduce(
    //    (str, key, i) => str + (i === 0 ? '\n' : '') + `  ${key} = '${key}', \n`,
    //    '',
    //  );
    var paramsString = __spread(fragmentRouteNames).map(function (key) { return "  " + key + " = '" + key + "'"; }).join(",\n");
...
try {
    ...
    if (navigationroot_1 && outputpath_1) {
        ...
        console.log("expo start -i --localhost", commander_1.default.showLogs, process.argv);
        child_process_1.exec('where expo', function (err, stdout) { return console.log("expo path:", stdout); });
        var expoProcess_1 = child_process_1.exec('expo start -i --localhost');
        var initExpoProcess_1 = function () {
            ...
            expoProcess_1.on('exit', function () {
                ...
                expoProcess_1 = child_process_1.exec('expo start -i --localhost');
                initExpoProcess_1();
            });
        };
        if (expoProcess_1) {
